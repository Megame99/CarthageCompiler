...
carthage_int errorNumber;     /* Run-time error number = 0 by default (ANSI) */

/* External objects */
extern carthage_int syntaxErrorNumber /* number of syntax errors reported by the parser */;
extern carthage_int line; /* source code line number - defined in scanner.c */
...
extern carthage_nul startParser(void);
extern carthage_int startScanner(BufferPointer sc_buf);

static carthage_nul printParserError(char* fmt, ...);
static carthage_nul displayParser(BufferPointer ptrBuffer);
static carthage_lng getParserFileSize(char* fname);
static carthage_nul callGarbageCollector(void);
...
carthage_nul printParserError(carthage_chr* fmt, ...) {

	va_list ap;
	va_start(ap, fmt);

	(carthage_nul)vfprintf(stderr, fmt, ap);
	va_end(ap);
...
carthage_lng getParserFileSize(carthage_chr* fname) {
	FILE* input;
	carthage_lng flength;
	input = fopen(fname, "r");
	if (input == NULL) {
...
carthage_nul displayParser(Buffer* ptrBuffer) {
...
carthage_nul callGarbageCollector(carthage_nul) {
